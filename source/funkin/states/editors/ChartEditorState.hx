...
class ChartEditorState extends haxe.ui.backend.flixel.UIState
{
	...
	**/
	...
	// Caching and dirty flag for grid updates
	var strumNotesCache:Array<StrumNote> = [];
	var gridDirty:Bool = true;
	var lastGridHash:String = "";
	...
	public function reloadStrumShit():Void
	{
		...
		{
			var note:StrumNote;
			if (i < strumNotesCache.length && strumNotesCache[i] != null)
			{
				note = strumNotesCache[i];
				note.revive();
				note.x = 0;
				note.y = strumLine != null ? strumLine.y : 50;
			...
			{
				note = new StrumNote(0, GRID_SIZE * (i + 1), strumLine != null ? strumLine.y : 50, i % song.keys);
				note.setGraphicSize(GRID_SIZE, GRID_SIZE);
				note.playAnim('static', true);
				note.scrollFactor.set(1, 1);
				note.updateHitbox();
				note.alpha = 0;
				strumNotesCache.push(note);
			}
	...
	{
		if (strumNotesCache != null)
		{
			for (note in strumNotesCache)
			{
				if (note != null) note.destroy();
			}
			strumNotesCache.resize(0);
		}
		...
		for (event in song.events)
		{
			...
			if (endThing > event[0] && event[0] >= startThing)
			{
				...
				for (i in 0...event[1].length)
				{
					...
					var newEv:Array<Dynamic> = [];
					for (j in 0...eventToPush.length)
						newEv.push(eventToPush[j]);
					copiedEventArray.push(newEv);
				}
	...
	function prepareEventsUI():Void
	{
		...
		{
			var name:String = '';
			if (Reflect.hasField(def, 'name')) name = Reflect.field(def, 'name');
			else if (Reflect.hasField(def, 'event')) name = Reflect.field(def, 'event');
			else if (Reflect.hasField(def, 'eventName')) name = Reflect.field(def, 'eventName');
			else name = fallbackName;
			
			var description:String = '';
			if (Reflect.hasField(def, 'description')) description = Reflect.field(def, 'description');
			else if (Reflect.hasField(def, 'desc')) description = Reflect.field(def, 'desc');
		...
		for (i in 0...directories.length)
		{
			...
			for (file in FunkinAssets.readDirectory(directory))
			{
				...
				for (ext in 0...eventexts.length)
				{
					...
					{
						var content:String = FunkinAssets.getContent(path);
					...
					{
						eventStuff.push({name: fileToCheck, description: FunkinAssets.getContent(path)});
					}
		...
		ui.songDialog.eventDropdown.populateList([for (ev in eventStuff) {id: ev.name, text: (ev.name.length == 0 ? 'None' : ev.name)}]);
	...
	function reloadGridLayer()
	{
		...
		gridDirty = true; // force grid update after background rebuild
	...
	function getGridHash():String
	{
		...
		for (section in song.notes)
		{
			...
			{
				data += note[0] + "," + note[1] + "," + note[2] + "," + note[3] + ";";
			}
		}
		return data;
	}
	...
	{
		var newHash = getGridHash();
		if (newHash == lastGridHash && !gridDirty) return;
		...
		for (i in song.events)
		{
			if (nextEndThing > i[0] && i[0] >= nextStartThing)
			{
